...
#Actualización de escuela
class UpdateViewEscuelaExpediente(View):
	@method_decorator(login_required)
	def get(self, request, pk):
		template_name = "accounts/updateExpediente.html"
		#perfil = Perfil.objects.get(pk=pk)
		escuela = User.objects.get(pk=pk)
		perfil = Perfil.objects.get(user=escuela)

		EdicionEscuelaExpediente = EscuelaPerfilUpdateForm2(instance=perfil)

		context = {
			'perfil': perfil,
			'escuela': escuela,
			'EdicionEscuelaExpediente': EdicionEscuelaExpediente,
		}
		return render(request,template_name, context)
	def post(self, request, pk):
		escuela = User.objects.get(pk=pk)
		...
		EdicionEscuelaExpediente = EscuelaPerfilUpdateForm2(instance=perfil, files=request.FILES)

		if EdicionEscuelaExpediente.is_valid():
			EdicionEscuelaExpediente.save()
			messages.success(request, "Actualización exitosa")

		return redirect("accounts:UpdateViewEscuelaExpediente", pk=pk)